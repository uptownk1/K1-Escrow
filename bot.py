...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("payment_received"):
        ...
        await context.bot.send_message(
            escrow["group_id"],  # Send to escrow group
    ...
    if data.startswith("payment_not_received"):
        escrow["status"] = "payment_not_received"
        escrow["buyer_confirmed"] = False
        await context.bot.send_message(
            escrow["group_id"],  # Send to escrow group
            "Admin has confirmed that the payment was not received. Please resolve the issue."
        )
...
async def handle_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If price fetching fails (i.e., price is None)
    if price is None:
        logging.error(f"Failed to fetch crypto price for {crypto_symbol}.")
        await update.message.reply_text("Error fetching crypto price. Please use /escrow to cancel trade and try again later.")
        return

    # Calculate the crypto amount if price is available
