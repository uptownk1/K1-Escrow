...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await clear_previous_buttons(context, escrow)  # Clear previous buttons every step
    ...
    if data == "join_buyer" and not escrow["buyer_id"]:
        ...
        await query.message.reply_text("Waiting for seller to join...", reply_markup=create_escrow_buttons(escrow))
    ...
    if data == "join_seller" and not escrow["seller_id"]:
        ...
        await query.message.reply_text("Waiting for buyer to join...", reply_markup=create_escrow_buttons(escrow))
    ...
    if data == "seller_sent_goods" and user_id == escrow["seller_id"]:
        ...
        buttons = create_buttons([("Release Funds ✅", "buyer_release_funds")])
    ...
    if data == "buyer_release_funds" and user_id == escrow["buyer_id"]:
        ...
        await context.bot.send_message(
            ...
            f"💷 Amount Sent: £{escrow['fiat_amount']} ({escrow['crypto_amount']} {coin})\n"
            f"💸 Escrow Fee (5%): £{escrow['fiat_amount']*ESCROW_FEE_PERCENT/100:.2f}\n"
            f"🏦 Amount After Fee: £{escrow['fiat_amount']* (1 - ESCROW_FEE_PERCENT/100):.2f}\n"
...
async def handle_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🎟️ Ticket: {escrow['ticket']}\n📌 Status: Awaiting Payment ⏳\n"
        f"💷 Amount: £{amount} ({crypto_amount} {crypto})\nSend exact amount to:\n`{wallet}`",
        parse_mode="Markdown",
        reply_markup=create_buttons([
...
async def wallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    after_fee = amount * (1 - ESCROW_FEE_PERCENT/100)
    fee_amount = amount * ESCROW_FEE_PERCENT/100
    # Notify admin with release button
    await context.bot.send_message(
        ADMIN_GROUP_ID,
        f"🎟️ Ticket: {ticket}\n📌 Status: Awaiting Admin Release ⏳\n"
        f"💷 Amount Sent: £{amount} ({escrow['crypto_amount']} {coin})\n"
        f"💸 Escrow Fee (5%): £{fee_amount:.2f}\n"
        f"🏦 Amount After Fee: £{after_fee:.2f}\n"
        f"👤 Buyer: @{buyer_username}\n👤 Seller: @{seller_username}\n"
...
async def admin_sent_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        f"💷 Amount Sent: £{amount} ({escrow['crypto_amount']} {crypto})\n"
        f"💸 Escrow Fee (5%): £{fee_amount:.2f}\n"
        f"🏦 Amount After Fee: £{after_fee:.2f}\n"
        f"👤 Buyer: @{buyer_username}\n👤 Seller: @{seller_username}\n"
...
async def dispute_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        f"💷 Amount: {amount} ({escrow.get('crypto_amount','N/A')} {crypto})\n"
        f"👤 Buyer: @{buyer_username}\n👤 Seller: @{seller_username}\n"
